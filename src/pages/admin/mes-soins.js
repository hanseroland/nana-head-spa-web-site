...
import { GetMyAppointmentsHistory } from '@/apiCalls/appointments'; // Votre appel API
...
const mesSoins = () => {
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            setError(null);
            try {
                const response = await GetMyAppointmentsHistory();
                if (response.success) {
    ...
    return (
        ...
        >
            ...
            >
                Mon Historique de Soins
            </Typography>

            {loading && (
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '200px' }}>
                    <CircularProgress sx={{ color: theme.palette.primary.main }} />
                    <Typography sx={{ mt: 2, color: theme.palette.text.secondary }}>Chargement de votre historique...</Typography>
                </Box>
            )}
            ...
            {!loading && !error && history.length === 0 && (
                <Alert severity="info" sx={{ mb: 3 }}>
                    Vous n'avez pas encore de soins terminés enregistrés.
                </Alert>
            )}
